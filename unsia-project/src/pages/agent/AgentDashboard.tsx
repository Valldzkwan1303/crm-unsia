...
import {
    ...
    Download, X, Share2, Users, GraduationCap, Briefcase,
    MessageCircle, Instagram, Facebook, Music2, ChevronRight, PieChart as PieIcon, Eye, CheckCircle
} from 'lucide-react';
...
const AgentDashboard = () => {
    ...
    const handleActivate = async (id: number) => {
        ...
        try {
            ...
            toast.success("Akun diaktifkan! Mahasiswa kini berstatus Calon Mahasiswa.");
            fetchData();
        } catch (e) {
    ...
    const handleChatWA = (phone: string, name: string) => {
        const message = `Halo ${name}, saya Partner resmi UNSIA. Saya lihat kamu sudah terdaftar. Ada yang bisa saya bantu untuk proses pendaftaranmu?`;
    ...
    const handleAutoShare = (platform: string, slug: string) => {
        ...
        if (!code || code === 'N/A') {
            toast.error("Data pendaftaran belum siap, silakan refresh.");
            return;
        }

        const link = `${window.location.origin}/p/${code}?src=${slug}`;

        navigator.clipboard.writeText(link);

        if (slug === 'whatsapp') {
            const msg = encodeURIComponent(`Halo! Yuk daftar kuliah di UNSIA melalui link resmi saya di sini: ${link}`);
    ...
    const downloadQRCode = () => {
        ...
        img.onload = () => {
            ...
            downloadLink.download = `QR-UNSIA-${data.stats.agent_code}.png`;
        ...
        <div className="space-y-8 animate-in fade-in duration-700 pb-10">
                    ...
                    <div className="space-y-4 text-center md:text-left">
                        <div className="flex items-center justify-center md:justify-start gap-3">
                            <div className="p-2 bg-white/10 rounded-xl backdrop-blur-md">
                                {isSGS ? <GraduationCap size={24} /> : <Briefcase size={24} />}
                            </div>
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-200">
                                {isSGS ? 'Student Ambassador' : 'Official Partner'}
                            </span>
                        </div>
                        <h1 className="text-4xl font-black tracking-tight">Halo, {stats.agent_name}!</h1>
                        <p className="text-blue-100 font-medium max-w-md italic">"{isSGS ? 'Bantu temanmu kuliah, kumpulkan poin beasiswamu.' : 'Pantau performa referral dan buat link pelacak otomatis Anda.'}"</p>
                        <button
                            type="button"
                            onClick={() => setShowLinkModal(true)}
                            className="bg-white text-[#002855] px-8 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-yellow-400 transition-all flex items-center gap-2 mt-4 mx-auto md:mx-0"
                        >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex justify-between items-center hover:shadow-xl transition-all group">
                    <div>
                        <span className="text-[10px] text-slate-400 font-black uppercase tracking-widest">{isSGS ? 'Tabungan Beasiswa' : 'Saldo Komisi'}</span>
            ...
            <div className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-2xl shadow-blue-900/5">
                <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-white">
                    <div><h3 className="text-xl font-black text-[#002855] uppercase tracking-tight leading-none">Monitoring Lead</h3><p className="text-slate-400 text-xs font-medium mt-2 italic">Kelola verifikasi pendaftar Anda</p></div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-slate-50 text-sm font-bold">
                            {referrals.length === 0 ? (
                                <tr><td colSpan={4} className="p-20 text-center text-slate-400 italic">Belum ada mahasiswa yang masuk.</td></tr>
                                ...
                                <tr key={i} className="hover:bg-blue-50/30 transition-colors">
                                    ...
                                    <td className="p-6 text-center">
                                        {item.registration_fee_proof ? (
                                            <button
                                                type="button"
                                                onClick={() => { setSelectedProof(item.registration_fee_proof!); setShowProofModal(true); }}
                                        ...
                                        <div className="flex justify-center gap-2">
                                            ...
                                            {item.status === 'lead' && item.registration_fee_proof ? (
                                                <button
                                                    ...
                                                    className="bg-[#002855] text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase shadow-lg hover:bg-blue-800 transition-all"
                                                >
                                                    Aktifkan Akun
                                                </button>
                                            ) : (['test_passed', 'calon_maba', 'awaiting_payment'].includes(item.status)) ? (
                                                <button
                                                    type="button"
                                                    onClick={() => handleChatWA(item.phone, item.name)}
                                                    className="bg-[#25D366] text-white px-5 py-2 rounded-xl text-[9px] font-black uppercase flex items-center gap-2 shadow-lg hover:bg-[#128C7E] transition-all"
                                                >
                                                    <MessageCircle size={14} /> WA
                                                </button>
                                            ) : item.status === 'active' ? (
                                                <div className="p-2 bg-emerald-50 text-emerald-600 rounded-xl" title="Sudah Closing">
                                                    <CheckCircle size={16} />
                                                </div>
                                            ) : (
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
                    <div className="bg-white w-full max-w-lg rounded-[2.5rem] p-4 shadow-2xl relative animate-in zoom-in-95">
                        ...
                        <div className="p-2 text-center">
                            <img
                                src={`http://127.0.0.1:8000/storage/${selectedProof}`}
                                alt="Bukti Transfer"
                                className="w-full h-auto rounded-[2rem] shadow-inner max-h-[70vh] object-contain"
                            />
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#002855]/20 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-white w-full max-w-md rounded-[3rem] p-10 shadow-2xl border border-slate-100 animate-in zoom-in-95">
                        ...
                        <div className="space-y-4">
                            <p className="text-slate-500 text-sm font-medium italic mb-6 text-center leading-relaxed">Pilih platform di bawah. Sistem akan menyisipkan kode pelacak secara otomatis.</p>
                            <div className="grid grid-cols-1 gap-3">
                                {shareOptions.map((opt) => (
                                    <button
                                        key={opt.name}
                                        // Kirim Nama (WhatsApp) dan Slug (whatsapp)
                                        onClick={() => handleAutoShare(opt.name, opt.slug)}
                                        className="w-full flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:border-blue-500 hover:bg-blue-50 transition-all group text-left"
                                        title={`Bagikan ke ${opt.name}`}
                                    >
                                        <div className={`p-3 ${opt.color} text-white rounded-xl shadow-lg`}>
                                            {opt.icon}
                                        </div>
                                        <div className="flex-1">
                                            <p className="font-black text-[#002855] text-sm leading-none">{opt.name}</p>
                                            <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">
                                                {opt.slug === 'whatsapp' || opt.slug === 'facebook' ? 'Buka Aplikasi' : 'Salin Link Pelacak'}
                                            </p>
                                        </div>
                            ...
                            {generatedLink && (
                                <div className="mt-8 p-5 bg-blue-50 rounded-2xl border border-blue-100 animate-in slide-in-from-bottom-2">
                                    <p className="text-[10px] font-black text-blue-400 uppercase tracking-widest mb-2">Link Terakhir Dibuat:</p>
