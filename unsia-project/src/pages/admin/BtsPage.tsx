...
import { Search, Loader2, ArrowLeft, Trash2, Edit, MapPin, ChevronRight, Users, Target, School, Map } from 'lucide-react';
...
const BtsPage = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error("Gagal muat data petugas BTS"); 
    } finally { 
      setLoading(false); 
    }
  };
  ...
  const filteredStaff = staff.filter(s => 
    ...
    (s.bts_id && s.bts_id.toLowerCase().includes(searchQuery.toLowerCase())) ||
    (s.unit && s.unit.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  const totalPetugas = staff.length;
  const wilayahTercover = new Set(staff.filter(s => s.unit).map(s => s.unit.trim().toLowerCase())).size;
        ...
        <div className="max-w-3xl mx-auto animate-in slide-in-from-bottom-4 duration-500 pt-8 pb-20 text-left">
           ...
           <div className="bg-white border border-slate-100 rounded-3xl p-10 shadow-2xl">
              <h2 className="text-2xl font-black text-[#002855] uppercase tracking-tight mb-2">
                  {modalMode === 'create' ? 'Daftarkan Petugas BTS' : 'Edit Petugas'}
              </h2>
              <p className="text-slate-500 text-sm font-medium mb-10">Lengkapi data di bawah ini untuk mendaftarkan personil tim sosialisasi lapangan (Back to School).</p>
              
              <form onSubmit={handleSave} className="space-y-6">
                  <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Nama Lengkap Petugas <span className="text-red-500">*</span></label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest">ID Petugas BTS <span className="text-red-500">*</span></label>
                        <input required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-semibold text-slate-700 outline-none focus:border-[#002855] focus:ring-4 focus:ring-[#002855]/10 transition-all placeholder:text-slate-300" value={formData.bts_id} onChange={e => setFormData({...formData, bts_id: e.target.value})} placeholder="Contoh: BTS-001" />
                    </div>
                    <div className="space-y-2">
                        <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Unit / Wilayah <span className="text-red-500">*</span></label>
                        <input required className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-semibold text-slate-700 outline-none focus:border-[#002855] focus:ring-4 focus:ring-[#002855]/10 transition-all placeholder:text-slate-300" value={formData.unit} onChange={e => setFormData({...formData, unit: e.target.value})} placeholder="Contoh: Jakarta Barat" />
                    </div>
                  </div>
                  <div className="space-y-2">
                      <label className="text-xs font-black text-slate-500 uppercase tracking-widest">Email Resmi <span className="text-red-500">*</span></label>
                      <input required type="email" className="w-full bg-white border-2 border-slate-100 rounded-xl px-5 py-4 text-sm font-semibold text-slate-700 outline-none focus:border-[#002855] focus:ring-4 focus:ring-[#002855]/10 transition-all placeholder:text-slate-300" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="petugas@unsia.ac.id" />
                  </div>
    ...
    <div className="bg-[#F8FAFC] min-h-screen p-4 md:p-8 space-y-8 animate-in fade-in duration-700 text-left -mx-4 md:-mx-8 -mt-4 md:-mt-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 pt-4">
         <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs font-bold text-blue-600 uppercase tracking-widest">Petugas BTS</p>
            </div>
            <h1 className="text-3xl font-black text-[#002855] uppercase tracking-tight mb-2 leading-none">Petugas Back To School</h1>
            <p className="text-slate-500 text-sm font-medium">Manajemen Tim Sosialisasi Lapangan & Perwakilan Daerah.</p>
         </div>
         <button type="button" onClick={() => { setModalMode('create'); setFormData({id:0,name:'',email:'',bts_id:'',unit:''}); setViewState('form'); }} className="bg-[#002855] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-md flex items-center gap-2 hover:bg-blue-800 transition-all">
            <MapPin size={18}/> Tambah Petugas
         </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
         <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group">
            <div><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Petugas BTS</p><h3 className="text-3xl font-black text-[#002855]">{totalPetugas}</h3></div>
            <div className="p-3.5 bg-blue-50 text-blue-600 rounded-2xl group-hover:scale-110 transition-transform"><Users size={24}/></div>
         </div>
         <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group">
            <div><p className="text-xs font-bold text-amber-500 uppercase tracking-wider mb-1">Wilayah Tercover</p><h3 className="text-3xl font-black text-amber-600">{wilayahTercover}</h3></div>
            <div className="p-3.5 bg-amber-50 text-amber-500 rounded-2xl group-hover:scale-110 transition-transform"><Map size={24}/></div>
         </div>
         <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group">
            <div><p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Total Leads Lapangan</p><h3 className="text-3xl font-black text-emerald-600">{totalLeads}</h3></div>
      ...
      <div className="flex justify-end">
        <div className="relative w-full md:w-96">
          ...
          <input className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3.5 text-sm font-semibold shadow-sm outline-none focus:border-[#002855] focus:ring-2 focus:ring-[#002855]/10 transition-all" placeholder="Cari nama atau ID Petugas..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-slate-50 border-b border-slate-100 text-xs font-bold uppercase text-slate-500 tracking-wider">
                <tr>
                   ...
                   <th className="px-6 py-5">ID / Unit Wilayah</th>
                 ...
                 {filteredStaff.length === 0 ? (
                    <tr>
                       <td colSpan={4}>
                          <div className="flex flex-col items-center justify-center py-20 text-center">
                             ...
                             <h3 className="text-lg font-bold text-[#002855] mb-1">Belum Ada Petugas</h3>
                             <p className="text-slate-500 text-sm">Tidak ada petugas lapangan dalam program BTS ini.</p>
                          </div>
                       </td>
                    </tr>
                 ) : (
                    filteredStaff.map(s => (
                       <tr key={s.id} className="hover:bg-slate-50/80 transition-colors group">
                          ...
                          <td className="px-6 py-5">
                             <p className="text-sm font-black text-slate-600 mb-1">{s.bts_id}</p>
                             <span className="inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border bg-amber-50 text-amber-600 border-amber-100">
                                {s.unit || 'Wilayah Umum'}
                             </span>
                          </td>
        ...
        <div className="md:hidden p-4 space-y-4 bg-[#F8FAFC]">
            {filteredStaff.length === 0 ? (
               <div className="bg-white rounded-3xl p-8 text-center border border-slate-100 shadow-sm">
                  ...
                  <p className="text-[#002855] font-bold text-lg">Belum Ada Petugas</p>
                  <p className="text-sm text-slate-500 mt-1">Tidak ada petugas lapangan dalam program BTS ini.</p>
               </div>
            ) : (
               filteredStaff.map(s => (
                  <div key={s.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm relative group hover:border-[#002855]/20 transition-all">
                     ...
                     <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 mb-5">
                        <div className="flex justify-between items-center mb-3 pb-3 border-b border-slate-200/60">
                           <div>
                              <p className="text-[10px] text-slate-400 font-bold uppercase mb-0.5">ID Petugas</p>
                              <p className="text-sm font-black text-slate-600">{s.bts_id}</p>
                           </div>
                           <div className="text-right">
                              <p className="text-[10px] text-slate-400 font-bold uppercase mb-1">Unit Wilayah</p>
                              <span className="inline-flex px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border bg-amber-50 text-amber-600 border-amber-100">
                                 {s.unit || 'Wilayah Umum'}
                              </span>
                           </div>
